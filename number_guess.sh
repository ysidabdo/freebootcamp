...
# Number Guessing Game - A simple interactive guessing game
# Uses PostgreSQL database to store user statistics
...
GAME() {
  ...
  while [[ $GUESSED = 0 ]]; do
    ...
    if [[ ! $GUESS =~ ^[0-9]+$ ]]; then
      ...
      echo -e "You guessed it in $TRIES tries. The secret number was $((TRIES-1)). Nice job!"
      #insert into db
